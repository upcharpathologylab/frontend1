...
function FieldLabel({ children }) {
  return <span className="mb-1.5 block text-sm font-black text-navy-950 md:mb-2 md:text-base">{children}</span>;
}
  ...
  return (
    <label className="signin-phone-field">
      ...
      <div className={`signin-phone-input flex h-10 overflow-hidden rounded-lg border bg-white md:h-12 ${error ? "border-red-300" : "border-blue-100"}`}>
        ...
        <input name="phone" value={value} onChange={onChange} inputMode="numeric" placeholder="Enter your phone number" className="min-w-0 flex-1 px-3 text-sm font-bold text-navy-900 outline-none placeholder:text-navy-500 md:px-5 md:text-base" />
        <div className="grid w-10 place-items-center text-navy-700 md:w-14">
...
function Divider() {
  return (
    <div className="my-3 flex items-center gap-4 text-sm font-black text-navy-950 md:my-5 md:gap-5">
...
function SignInForm({ onModeChange, onSuccess }) {
  ...
  return (
    <form onSubmit={submit} noValidate className="auth-card auth-card-signin">
      <div className="grid gap-3 md:gap-5">
        ...
        {!resetMode ? (
          <>
            ...
            <button type="button" onClick={sendReset} className="justify-self-end text-sm font-black text-upchar-green md:text-base">
              Forgot Password?
            </button>
          </>
        ) : (
      ...
      {resetMode ? (
        <button type="button" onClick={savePassword} disabled={submitting} className="auth-primary-btn mt-4 md:mt-5">
          {submitting ? "Saving..." : "Save New Password"}
        </button>
      ) : (
        <button type="submit" disabled={submitting} className="auth-primary-btn mt-4 md:mt-5">
