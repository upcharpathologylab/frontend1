...
const maxHomeBannerImageSize = 200 * 1024;
...
function BannerForm({ item, onClose, onSave }) {
  ...
  const uploadImage = async (file, key = "bannerImage") => {
    if (!file) return;
    if (file.size > maxHomeBannerImageSize) {
      setError("Banner image size must be 200KB or less.");
      return;
    }
    try {
      setUploading(true);
      setError("");
      const result = await uploadContentImage(file, "homeBanner");
